...
WITH tech_source AS (
    SELECT 
        ...
        r.ProjectionYear,
        r.LicensorId
    FROM {{ source('media_dm_staging', 'OrderInvoiceDetail') }} r
),
sap_data AS (
...
SELECT
    -- 1. InvoiceKey
    {{ get_source_id_key('T', 'r.RecordId') }} as InvoiceKey,
    -- 2. SOLItem
...
FROM tech_source r
JOIN {{ source('media_dm_staging', 'OrderInvoice') }} ri ON ri.Id = r.OrderInvoiceId
LEFT JOIN {{ source('media_dm_staging', 'InvoiceStatus_lu') }} s ON s.Id = ri.InvoiceStatusId
LEFT JOIN {{ source('media_dm_staging', 'Fee_lu') }} f ON f.Id = ri.FeeId
